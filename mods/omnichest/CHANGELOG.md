...
- 設置済みの**チェスト / トラップチェスト / ラージチェスト / 樽 / シュルカーボックス /
  エンダーチェスト**に照準（クロスヘア）を合わせ、**ピークキーを押している間だけ**
  中身をポップアップ表示する。チェストを開く必要はない。
- 使うには **設定 → 表示 / UI → 「コンテナ ピーク」を ON** にする。ON にするまでは何も起こらず、
  キーを押しても反応しない。
- ポップアップの見た目は既存の **Alt ホバー プレビューと同じ**。中身のグリッド、
  使用スロット数と総個数のサマリまでそのまま同じ描画を使っている。
- **ホッパー / ディスペンサー / ドロッパー / クラフターは対象外**。これらは「保管」ではなく
  「機構」なので、レッドストーン作業中に不要なポップアップが出ないようにした。

### ★ これは「今の中身」ではなく「最後に開いたときの中身」である

Minecraft はコンテナの中身をクライアントへ送っていないため、**開かずに現在の中身を知ることは
原理的にできない**。この機能が表示しているのは、**倉庫検索がすでに記録しているスナップショット**
...
- エンダーチェストの中身はどのブロックから開いても同じなので、**最後に開いたエンダーチェストの
  内容を、どのエンダーチェストを覗いても表示する**。A を開いたあとに未開封の B を覗いても
  「未登録」にはならない。
...
### Fixed — ラージチェストのポップアップがホットバーに重なる

- **これまで**: ラージチェスト（54 スロット = 6 行）を覗くと、ポップアップ下端がホットバーに
  重なり、サマリ行（`3 / 54 · ×4` など）が読めなかった。配置計算が「画面の下端」だけを見ていて、
  **画面下部の HUD（ホットバー・経験値バー・体力/満腹度・防具/空気・持ち替えアイテム名）の
  存在を考慮していなかった**のが原因。
- **これから**: 下部 HUD の帯を避けて配置する。下に収まらない場合は**クロスヘアの上側へ反転**する
  （単に上へずらすと狙っているブロックを覆ってしまうため）。上下どちらにも入らない縦長のときは
  安全帯の中央へ置く。
- **クロスヘアは常に見える**。ポップアップをバニラのクロスヘアより**手前ではなく奥**に描くよう
  変更したので、仮にポップアップとクロスヘアが重なっても照準は見えたままで、対象ブロックを
  狙い続けられる。
- 27 スロット・樽・シュルカー・未登録表示の見え方は従来どおり。

### キーについて — 既定は `Z`

- ピークキーの既定は **`Z`**。押している間だけポップアップが出る。
- `Z` を選んだ理由: Minecraft の既定キーに存在せず、この MOD の既存キー（`G` 倉庫検索 /
  `J` 振り分け / `H` 自動投入 / 中クリック スロットロック）とも、同梱の VisualizeGate（`V`）
  とも衝突しないため。移動キー（WASD）からも離れているので、押しっぱなしでも移動に混ざらない。
