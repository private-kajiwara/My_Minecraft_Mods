...
### ★ 既知の衝突 — 既定キー（左 Alt）と Alt+C / Alt+D

- ピークキーの既定は**左 Alt**。**左 Alt を押している最中に `C` または `D` を押すと、
  既存の Alt+C（ディメンションメニュー）／ Alt+D（ワールド上の全ピン解除）が発火する。**
- これは既存機能の挙動を変えないためにあえて抑止していない。気になる場合は次のどちらでも回避できる:
  1. **ピークキーを別のキーに再割当する**（操作設定 → OmniChest → 「コンテナの中身を覗く」）。
  2. **設定「ディメンションメニュー (Alt+C)」を OFF にする**（Alt+C だけ止まる。メニュー自体は
     再割当キーから開ける）。
- ピークキーを**未割当**にすると、設定が ON のままでも機能は発火しない。
